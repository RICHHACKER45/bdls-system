...
@section ('content')
    ...
    <div
        id="tab-pending"
        class="tab-content {{ session('active_tab', 'pending') == 'pending' ? 'block' : 'hidden' }}"
    >
        ...
        <div class="mb-6 rounded-xl border border-slate-200 bg-white p-4 shadow-sm">
            ...
            >
                <input
                    ...
                    class="min-w-[200px] flex-1 rounded-lg border border-slate-300 px-4 py-2 outline-none focus:ring-2 focus:ring-slate-900 focus:outline-none"
                />
                <select
                    name="sort"
                    onchange="this.form.submit()"
                    class="rounded-lg border border-slate-300 bg-slate-50 px-4 py-2 focus:outline-none"
                >
                ...
                <button
                    type="submit"
                    class="shrink-0 rounded-lg bg-slate-900 px-6 py-2 font-bold text-white transition-all hover:bg-slate-800 active:scale-95"
                >
                    Search
                </button>
            </form>
        ...
        <div class="mb-6 flex gap-2 overflow-x-auto pb-2">
            <button
                onclick="showSubTab('sub-pending', this)"
                class="sub-tab-btn rounded-full bg-slate-900 px-5 py-2 text-sm font-bold whitespace-nowrap text-white transition-all"
            >
                Under Review ({{ $pendingAccounts->count() }})
            </button>
            <button
                onclick="showSubTab('sub-approved', this)"
                class="sub-tab-btn rounded-full bg-slate-200 px-5 py-2 text-sm font-bold whitespace-nowrap text-slate-700 transition-all hover:bg-slate-300"
            >
                Approved ({{ $approvedAccounts->count() }})
            </button>
            <button
                onclick="showSubTab('sub-rejected', this)"
                class="sub-tab-btn rounded-full bg-slate-200 px-5 py-2 text-sm font-bold whitespace-nowrap text-slate-700 transition-all hover:bg-red-200 hover:text-red-700"
            >
        ...
        <div id="sub-pending" class="sub-tab-content grid grid-cols-1 gap-6 xl:grid-cols-2">
            @forelse ($pendingAccounts as $user)
                <div
                    class="flex flex-col gap-5 rounded-xl border border-slate-200 bg-white p-5 shadow-sm transition-all hover:shadow-md sm:flex-row"
                >
                    <!-- TASK 1: Clickable Images -->
                    <div class="flex shrink-0 gap-2 sm:flex-col">
                        <div class="group relative">
                            <img
                                src="{{ asset('storage/' . $user->id_photo_path) }}"
                                class="h-20 w-20 cursor-pointer rounded-lg border border-slate-200 object-cover transition-all group-hover:opacity-75 sm:h-24 sm:w-24"
                                onclick="openModal(this.src, 'Valid ID')"
                            />
                            <span
                                class="absolute right-1 bottom-1 rounded bg-slate-900/60 px-1 text-[8px] font-bold text-white"
                                >ID</span
                            >
                        </div>
                        <div class="group relative">
                            <img
                                src="{{ asset('storage/' . $user->selfie_photo_path) }}"
                                class="h-20 w-20 cursor-pointer rounded-lg border border-slate-200 object-cover transition-all group-hover:opacity-75 sm:h-24 sm:w-24"
                                onclick="openModal(this.src, 'Selfie')"
                            />
                            <span
                                class="absolute right-1 bottom-1 rounded bg-slate-900/60 px-1 text-[8px] font-bold text-white"
                                >SELFIE</span
                    ...
                    <div class="flex flex-1 flex-col justify-between">
                        <div>
                            ...
                            <h3
                                class="mb-1 text-xl leading-tight font-black tracking-tight text-slate-900 uppercase"
                            >
                        ...
                        <div class="flex gap-2">
                            <form
                                action="{{ route('admin.approve_account', $user->id) }}"
                                method="POST"
                                class="flex-1"
                            >
                                @csrf
                                <button
                                    type="submit"
                                    class="w-full rounded-lg bg-slate-900 py-2.5 text-[10px] font-black tracking-widest text-white uppercase shadow-sm transition-all hover:bg-slate-800 active:scale-95"
                                >
                                    Approve
                                </button>
                            </form>
                            <button
                                ...
                                class="flex-1 rounded-lg border border-red-200 bg-red-50 py-2.5 text-[10px] font-black tracking-widest text-red-600 uppercase transition-all hover:bg-red-100 active:scale-95"
                            >
                                Reject
                            </button>
                        </div>
        ...
        <div id="sub-approved" class="sub-tab-content hidden space-y-3">
            @forelse ($approvedAccounts as $user)
                ...
                >
                    <summary
                        class="flex cursor-pointer list-none items-center justify-between p-4 transition-all hover:bg-slate-50"
                    >
                    ...
                    <div
                        class="grid grid-cols-1 gap-6 border-t border-slate-50 bg-slate-50/30 p-5 text-xs md:grid-cols-2 lg:grid-cols-3"
                    >
                        <div>
                        ...
                        <div
                            class="flex items-center justify-between border-t border-slate-100 pt-2 lg:col-span-3"
                        >
                            ...
                            <div class="flex items-center gap-3">
                                ...
                                <div class="flex gap-2">
                                    <img
                                        src="{{ asset('storage/' . $user->id_photo_path) }}"
                                        class="h-8 w-8 cursor-pointer rounded border border-slate-200 object-cover"
                                        onclick="openModal(this.src, 'ID')"
                                    />
                                    <img
                                        src="{{ asset('storage/' . $user->selfie_photo_path) }}"
                                        class="h-8 w-8 cursor-pointer rounded border border-slate-200 object-cover"
                                ...
                                <div class="flex items-center gap-2 border-l border-slate-200 pl-3">
                                    ...
                                    <button
                                        ...
                                        class="flex items-center gap-1 rounded border border-red-200 bg-red-50 px-3 py-1.5 text-[9px] font-black tracking-widest text-red-600 uppercase shadow-sm transition-all hover:bg-red-100 active:scale-95"
                                    >
        ...
        <div id="sub-rejected" class="sub-tab-content grid hidden grid-cols-1 gap-6 xl:grid-cols-2">
            @forelse ($rejectedAccounts as $user)
                <div
                    class="flex flex-col justify-between rounded-xl border border-l-4 border-slate-200 border-l-red-500 bg-white p-5 shadow-sm"
                >
                    <div>
                        <div class="mb-4 flex items-start justify-between">
                            <div>
                                <h3
                                    class="text-lg leading-tight font-black tracking-tight text-slate-900 uppercase"
                                >
                            ...
                            <span
                                class="rounded bg-red-100 px-2 py-1 text-[9px] font-black tracking-widest text-red-700 uppercase shadow-sm"
                                >Rejected</span
                            >
                        </div>
                    ...
                    <button
                        type="button"
                        onclick="openDeleteModal('{{ $user->id }}')"
                        class="w-full rounded-lg bg-red-600 py-2.5 text-[10px] font-black tracking-widest text-white uppercase shadow-md transition-all hover:bg-red-700 active:scale-95"
                    >
                        Delete Account
                    </button>
                </div>
    ...
    <div
        id="tab-queue"
        class="tab-content {{ session('active_tab') == 'queue' ? 'block' : 'hidden' }}"
    >
        <!-- TASK 2: Horizontal sub-tab for Active vs Received -->
        <div class="mt-6 mb-6 flex gap-2 overflow-x-auto pb-2">
            <button
                onclick="showSubTab('queue-active', this)"
                class="sub-tab-btn rounded-full border border-slate-900 bg-slate-900 px-5 py-2 text-sm font-bold whitespace-nowrap text-white transition-all"
            >
                Active Queue ({{ $activeQueue->count() }})
            </button>
            <button
                onclick="showSubTab('queue-received', this)"
                class="sub-tab-btn rounded-full border border-transparent bg-slate-200 px-5 py-2 text-sm font-bold whitespace-nowrap text-slate-700 transition-all hover:bg-slate-300"
            >
        ...
        <div
            id="queue-active"
            class="sub-tab-content overflow-hidden rounded-xl border border-slate-200 bg-white shadow-sm"
        >
            <table class="w-full border-collapse text-left">
                <thead>
                    <tr
                        class="border-b border-slate-200 bg-slate-50 text-[10px] tracking-[0.15em] text-slate-500 uppercase"
                    >
    ...
    @endphp
                        <tr class="transition-colors hover:bg-slate-50">
                            <td class="p-4 text-xl font-black tracking-tighter text-slate-900">
                                {{ $queue->queue_number }}
                            </td>
                            <td class="p-4">
                            ...
                            <td class="p-4 text-xs font-bold text-slate-700 uppercase">
                                {{ $queue->documentType->name ?? 'N/A' }}
                            </td>
                            <td class="p-4">
                                <!-- FIXED STATUS COLORS -->
                                <span
            ...
            "
                            ...
                            <td class="flex justify-end gap-2 p-4 text-right">
                                ...
                                @if ($btnLabel !== '')
                                    <button
                                        type="button"
                                        onclick="openStatusModal('{{ $queue->id }}', '{{ $nextStatus }}', '{{ $btnLabel }}')"
                                        class="rounded-lg bg-slate-900 px-4 py-2 text-[10px] font-black tracking-widest text-white uppercase shadow-sm transition-all hover:bg-slate-800 active:scale-95"
                                    >
                                        {{ $btnLabel }}
                                    </button>
                                @endif
                                ...
                                @if ($rawStatus === 'pending' || $rawStatus === 'processing')
                                    <button
                                        type="button"
                                        onclick="openStatusModal('{{ $queue->id }}', 'rejected', 'Reject Request')"
                                        class="rounded-lg border border-red-200 bg-red-50 px-4 py-2 text-[10px] font-black tracking-widest text-red-600 uppercase shadow-sm transition-all hover:bg-red-100 active:scale-95"
                                    >
                                        Reject
                                    </button>
                                @endif
                                ...
                                @if ($btnLabel === '' && $rawStatus !== 'pending' && $rawStatus !== 'processing')
                                    <span class="mt-2 text-xs font-bold text-slate-400 italic"
                                        >No Action</span
                                    >
                                @endif
                            </td>
                    ...
                    @empty
                        <tr>
                            <td
                                colspan="5"
                                class="p-12 text-center font-bold text-slate-400 italic"
                            >
        ...
        <div
            id="queue-received"
            class="sub-tab-content hidden overflow-hidden rounded-xl border border-slate-200 bg-white shadow-sm"
        >
            ...
            >
                ...
                <button
                    onclick="openLogbookModal('{{ route('admin.queue.print_logbook') }}')"
                    class="flex items-center gap-2 rounded-lg bg-slate-900 px-4 py-2 text-[10px] font-black tracking-widest text-white uppercase shadow-sm transition-all hover:bg-slate-800 active:scale-95"
                >
                ...
                <tbody class="divide-y divide-slate-100">
                    @forelse ($receivedQueue as $queue)
                        <tr class="bg-slate-50/50">
                            <td class="p-4 text-xl font-black tracking-tighter text-slate-400">
                                {{ $queue->queue_number }}
                            </td>
                            ...
                            <td class="p-4 text-xs font-bold text-slate-500">
                                {{ $queue->released_at ? $queue->released_at->format('M d, Y h:i A') : 'N/A' }}
                            </td>
                            <td class="p-4 text-right">
                                <span
                                    class="rounded-full border border-green-200 bg-green-100 px-3 py-1 text-[9px] font-black tracking-widest text-green-700 uppercase"
                                >
                                    Received
                                </span>
                            </td>
    ...
    <div
        id="tab-walkin"
        class="tab-content {{ session('active_tab') == 'walkin' ? 'block' : 'hidden' }}"
    >
        <div
            class="mx-auto max-w-3xl rounded-xl border border-slate-200 bg-white p-6 shadow-sm md:p-8"
        >
            <div class="mb-6 text-center">
                <div
                    class="mx-auto mb-3 flex h-14 w-14 items-center justify-center rounded-full bg-slate-100 text-slate-900"
                >
            ...
            <form
                action="{{ route('admin.walkin.search') }}"
                method="POST"
            ...
            >
                ...
                <div class="flex-1">
                    ...
                    <input
                        type="text"
                        name="contact_number"
                        value="{{ session('walkin_search_number') }}"
                        required
                        placeholder="09XXXXXXXXX"
                        oninput="this.value = this.value.replace(/[^0-9]/g, '')"
                        maxlength="11"
                        class="w-full rounded-lg border border-slate-300 px-5 py-3 text-center font-mono text-xl font-bold tracking-widest transition-all outline-none focus:ring-2 focus:ring-slate-900 sm:text-left"
                    />
                </div>
                <button
                    type="submit"
                    class="shrink-0 rounded-lg bg-slate-900 px-8 py-3 text-xs font-black tracking-widest whitespace-nowrap text-white uppercase shadow-sm transition-all hover:bg-slate-800 active:scale-95"
                >
                    I-Search
                </button>
            </form>
            ...
            @if (session('walkin_searched'))
                <div class="mt-8 border-t border-slate-100 pt-6">
                    @if (session('walkin_user'))
                        ...
                        <form
                            action="{{ route('admin.walkin.store') }}"
                            method="POST"
                        ...
                        >
                            ...
                            <div class="flex flex-col gap-3 sm:flex-row">
                                <select
                                    ...
                                    required
                                    class="min-w-0 flex-1 rounded-lg border border-green-300 bg-white px-4 py-3 text-sm font-bold text-slate-700 outline-none focus:ring-2 focus:ring-green-600"
                                >
                                ...
                                <button
                                    type="submit"
                                    class="shrink-0 rounded-lg bg-green-600 px-6 py-3 text-[10px] font-black tracking-widest whitespace-nowrap text-white uppercase shadow-sm transition-all hover:bg-green-700 active:scale-95"
                                >
                                    Create Request
                                </button>
                            </div>
                    ...
                    @else
                        ...
                        <form
                            action="{{ route('admin.walkin.store') }}"
                            method="POST"
                            class="rounded-xl border border-amber-200 bg-amber-50 p-5 md:p-6"
                        >
                            @csrf
                            <input
                                type="hidden"
                            ...
                            <div class="mb-4 grid grid-cols-1 gap-3 sm:grid-cols-2">
                                <input
                                    ...
                                    required
                                    class="w-full rounded-lg border border-amber-300 bg-white px-4 py-2.5 text-sm font-bold outline-none focus:ring-2 focus:ring-amber-600"
                                />
                                <input
                                    type="text"
                                    name="last_name"
                                ...
                                <div
                                    class="flex items-center rounded-lg border border-amber-300 bg-white px-3 focus-within:ring-2 focus-within:ring-amber-600"
                                >
                                    <span
                                        class="mr-2 text-xs font-bold tracking-widest text-slate-400 uppercase"
                                        >DOB*</span
                                    >
                                    <input
                                        type="date"
                                        name="date_of_birth"
                                        required
                                        class="w-full bg-transparent py-2.5 text-sm font-bold outline-none"
                                    />
                                </div>
                                <input
                                    type="text"
                            ...
                            <div
                                class="mt-2 flex flex-col gap-3 border-t border-amber-200 pt-4 sm:flex-row"
                            >
                                <select
                                    name="document_type_id"
                                    required
                                    class="min-w-0 flex-1 rounded-lg border border-amber-300 bg-white px-4 py-3 text-sm font-bold text-slate-700 outline-none focus:ring-2 focus:ring-amber-600"
                                >
                                    <option value="">-- Piliin ang Dokumento --</option>
                                    @foreach ($documents as $doc)
                                        <option value="{{ $doc->id }}">{{ $doc->name }}</option>
                                ...
                                <button
                                    type="submit"
                                    class="shrink-0 rounded-lg bg-amber-600 px-6 py-3 text-[10px] font-black tracking-widest whitespace-nowrap text-white uppercase shadow-sm transition-all hover:bg-amber-700 active:scale-95"
                                >
                                    Register & Create
                                </button>
                            </div>
    ...
    <div
        ...
        class="tab-content {{ session('active_tab') == 'announcements' ? 'block' : 'hidden' }}"
    >
        <div class="mx-auto mt-8 max-w-3xl">
            ...
            @endphp
                ...
                @if ($isCurfew || $errors->has('curfew'))
                    <div
                        class="mb-6 rounded-r-xl border-l-4 border-red-500 bg-red-50 p-4 shadow-sm"
                    >
                        <div
                            class="mb-1 flex items-center gap-2 text-xs font-black tracking-widest text-red-700 uppercase"
                        >
                ...
                <form action="{{ route('admin.announcements.broadcast') }}" method="POST">
                    ...
                    <div class="mb-4">
                        <label
                            ...
                            >Mensahe (Message Body) <span class="text-red-500">*</span></label
                        >
                        <textarea
                            name="message_body"
                    ...
                    <div class="flex justify-end pt-2">
                        <button
                            ...
                            id="broadcastBtn"
                            {{ $isCurfew ? 'disabled' : '' }}
                            class="bg-slate-900 hover:bg-slate-800 text-white font-black text-xs uppercase tracking-widest px-8 py-3.5 rounded-xl transition-all active:scale-95 shadow-md flex items-center gap-2 {{ $isCurfew ? 'opacity-50 cursor-not-allowed hover:bg-slate-900 active:scale-100' : '' }}"
                        >
                            Send Broadcast
                        </button>
                    </div>
    ...
    <div
        id="tab-audit"
        class="tab-content {{ session('active_tab') == 'audit' ? 'block' : 'hidden' }}"
    >
        <!-- SUB-TAB BUTTONS -->
        <div class="mt-6 mb-6 flex gap-2 overflow-x-auto pb-2">
            <button
                onclick="showSubTab('sub-audit-trail', this)"
                class="sub-tab-btn rounded-full border border-slate-900 bg-slate-900 px-5 py-2 text-sm font-bold whitespace-nowrap text-white transition-all"
            >
                System Audit Trail
            </button>
            <button
                onclick="showSubTab('sub-notif-history', this)"
                class="sub-tab-btn rounded-full border border-transparent bg-slate-200 px-5 py-2 text-sm font-bold whitespace-nowrap text-slate-700 transition-all hover:bg-slate-300"
            >
                Notification History
            </button>
            <button
                onclick="showSubTab('sub-generate-pdf', this)"
                class="sub-tab-btn rounded-full border border-transparent bg-slate-200 px-5 py-2 text-sm font-bold whitespace-nowrap text-slate-700 transition-all hover:bg-slate-300"
            >
                Generate Analytics
            </button>
        ...
        <div
            id="sub-audit-trail"
            class="sub-tab-content block overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm"
        >
            ...
            <div class="max-h-[600px] overflow-x-auto overflow-y-auto">
                <table class="relative w-full border-collapse text-left">
                    <thead class="sticky top-0 z-10">
                        <tr
                            class="border-b border-slate-200 bg-slate-100 text-[10px] tracking-[0.15em] text-slate-500 uppercase"
                        >
                    ...
                    <tbody class="divide-y divide-slate-100">
                        @forelse ($auditLogs as $log)
                            <tr class="transition-colors hover:bg-slate-50">
                                ...
                                <td class="p-4 text-xs font-bold text-slate-900 uppercase">
                                    {{ $log->admin->first_name ?? 'System' }} {{ $log->admin->last_name ?? '' }}
                                </td>
                                <td class="p-4">
                                    <span
                                        class="rounded bg-slate-900 px-2 py-1 text-[9px] font-black tracking-widest text-white uppercase shadow-sm"
                                        >{{ $log->action }}</span
                                    >
                                </td>
        ...
        <div
            id="sub-notif-history"
            class="sub-tab-content hidden overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm"
        >
            <div class="border-b border-slate-200 bg-slate-50 p-6">
                <h2 class="text-xl font-black tracking-tight text-slate-900 uppercase">
                    Notification History
                ...
                <table class="relative w-full border-collapse text-left">
                    ...
                    <tbody class="divide-y divide-slate-100">
                        @forelse ($notificationLogs as $notif)
                            <tr class="transition-colors hover:bg-slate-50">
                                <td class="p-4 text-xs font-bold text-slate-600">
                                    {{ \Carbon\Carbon::parse($notif->created_at)->format('M d, Y h:i A') }}
                                </td>
                                <td class="p-4">
                                ...
                                <td class="p-4">
                                    @if (strtolower($notif->channel) == 'sms')
                                        <span
                                            class="rounded border border-blue-200 bg-blue-100 px-2 py-1 text-[9px] font-black tracking-widest text-blue-700 uppercase shadow-sm"
                                            >SMS</span
                                        >
                                    @else
                                        <span
                                            class="rounded border border-purple-200 bg-purple-100 px-2 py-1 text-[9px] font-black tracking-widest text-purple-700 uppercase shadow-sm"
                                            >EMAIL</span
                                        >
                                    @endif
                                </td>
                                <td
                                    class="line-clamp-2 p-4 text-[11px] font-medium text-slate-600"
                                ...
                                <td class="p-4 text-right">
                                    @if (str_contains(strtolower($notif->status), 'sent'))
                                        <span
                                            class="rounded bg-green-100 px-2 py-1 text-[9px] font-black tracking-widest text-green-700 uppercase"
                                            >{{ $notif->status }}</span
                                        >
                                    @else
                                        <span
                                            class="rounded bg-red-100 px-2 py-1 text-[9px] font-black tracking-widest text-red-700 uppercase"
                ...
                >
                    ...
                    <div class="grid grid-cols-1 gap-4 sm:grid-cols-2">
                        <div class="rounded-xl border border-slate-200 bg-slate-50 p-4">
                            <label
                                class="mb-2 block text-[10px] font-black tracking-widest text-slate-400 uppercase"
                                >Piliin ang Buwan <span class="text-red-500">*</span></label
                            >
                            <select
                                name="report_month"
                                required
                                class="w-full cursor-pointer rounded-lg border border-slate-300 bg-white px-4 py-2.5 font-bold text-slate-700 outline-none focus:ring-2 focus:ring-slate-900"
                            >
                        ...
                        <div class="rounded-xl border border-slate-200 bg-slate-50 p-4">
                            <label
                                class="mb-2 block text-[10px] font-black tracking-widest text-slate-400 uppercase"
                                >Piliin ang Taon <span class="text-red-500">*</span></label
                            >
                            <select
                                name="report_year"
                                required
                    ...
                    <button
                        type="submit"
                        class="flex w-full items-center justify-center gap-2 rounded-xl bg-slate-900 py-4 text-xs font-black tracking-widest text-white uppercase shadow-md transition-all hover:bg-slate-800 active:scale-95"
                    >
    ...
    <div
        id="pdfModal"
        class="fixed inset-0 z-[200] hidden items-center justify-center bg-slate-900/90 p-4 backdrop-blur-sm transition-opacity sm:p-8"
    >
        <div
            class="flex h-[90vh] w-full max-w-5xl flex-col overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-2xl"
        >
            <!-- Modal Header -->
            <div
            ...
            >
                <h2
                    class="flex items-center gap-2 text-lg font-black tracking-tight text-slate-900 uppercase"
                >
                ...
                <button
                    ...
                    class="text-3xl leading-none font-bold text-slate-400 transition-all hover:text-red-600"
                >
                    &times;
                </button>
            </div>
            ...
            <div class="relative w-full flex-1 bg-slate-200">
                ...
                <iframe
                    name="pdfViewerFrame"
                    id="pdfViewerFrame"
                    onload="hideGlobalLoader()"
                    class="relative z-10 h-full w-full bg-white"
                ></iframe>
            </div>
            <!-- Modal Footer: WITH DOWNLOAD BUTTON -->
            <div
                class="flex items-center justify-between border-t border-slate-200 bg-slate-50 p-4"
            >
                <button
                    onclick="downloadPdfNow()"
                    class="flex items-center gap-2 rounded-xl bg-red-600 px-6 py-3 text-[10px] font-black tracking-widest text-white uppercase shadow-sm transition-all hover:bg-red-700 active:scale-95"
                >
                ...
                <button
                    onclick="closePdfModal()"
                    class="rounded-xl bg-slate-900 px-8 py-3 text-[10px] font-black tracking-widest text-white uppercase shadow-sm transition-all hover:bg-slate-800 active:scale-95"
                >
                    Isara
                </button>
            </div>
    ...
    <div
        id="statusModal"
        class="fixed inset-0 z-[110] flex hidden items-center justify-center bg-slate-900/80 p-4 backdrop-blur-sm"
    >
        <div
            class="w-full max-w-sm transform overflow-hidden rounded-2xl border border-slate-100 bg-white p-6 shadow-2xl transition-all"
        >
            <div class="mb-6 flex flex-col items-center text-center">
                <div
                    class="mb-4 flex h-16 w-16 items-center justify-center rounded-full bg-slate-100 text-slate-900"
                >
                ...
                >
                    Move to <span id="nextStatusLabel" class="text-blue-600">...</span>?
                </h3>
            ...
            <form id="statusForm" method="POST">
                ...
                <div class="flex flex-col gap-3">
                    <button
                        type="submit"
                        class="w-full rounded-xl bg-slate-900 py-3 text-xs font-black tracking-widest text-white uppercase shadow-md transition-all hover:bg-slate-800 active:scale-95"
                    >
                        Confirm Update
                    </button>
                    <button
    ...
    >
        <div
            class="w-full max-w-sm transform overflow-hidden rounded-2xl border border-red-100 bg-white p-6 shadow-2xl transition-all"
        >
            <div class="mb-6 flex flex-col items-center text-center">
                <div
            ...
            <form id="deleteForm" method="POST">
                ...
                <div class="flex flex-col gap-3">
                    <button
                        type="submit"
                        class="w-full rounded-xl bg-red-600 py-3 text-xs font-black tracking-widest text-white uppercase shadow-md transition-all hover:bg-red-700 active:scale-95"
                    >
                        Delete Permanently
                    </button>
                    <button
    ...
    <div
        id="imageModal"
        class="fixed inset-0 z-[120] flex hidden items-center justify-center bg-slate-900/90 p-4 backdrop-blur-sm"
    >
        <div class="relative w-full max-w-4xl overflow-hidden rounded-xl bg-white shadow-2xl">
            ...
            >
                <h3
                    id="modalTitle"
                    class="text-lg font-black tracking-tighter text-slate-900 uppercase"
                >
                    Preview
                </h3>
                <button
                    onclick="closeModal()"
                    class="text-2xl leading-none font-bold text-slate-400 transition-all hover:text-red-600"
                >
                    &times;
                </button>
            </div>
            ...
            <div class="border-t border-slate-200 bg-slate-50 p-4 text-right">
                <button
                    onclick="closeModal()"
                    class="rounded-lg bg-slate-900 px-10 py-3 text-[10px] font-black tracking-widest text-white uppercase transition-all hover:bg-slate-800 active:scale-95"
                >
                    Isara
                </button>
            </div>
    ...
    <div
        id="rejectModal"
        class="fixed inset-0 z-[110] hidden items-center justify-center bg-slate-900/80 p-4 backdrop-blur-sm"
    >
        <div
            class="w-full max-w-md transform overflow-hidden rounded-2xl border border-red-100 bg-white shadow-2xl transition-all"
        >
            <div
                class="flex items-center justify-between border-b border-red-100 bg-red-50 p-4 text-red-700"
            >
                ...
                <button
                    ...
                    class="text-2xl font-bold text-red-300 transition-all hover:text-red-700"
                >
                    &times;
                </button>
            </div>
            ...
            <form id="rejectForm" method="POST" action="">
                ...
                <div class="p-6">
                    ...
                    <input
                        ...
                        class="w-full rounded-xl border-2 border-slate-100 p-3 font-bold text-slate-900 placeholder-slate-300 transition-all outline-none focus:border-red-500 focus:ring-2 focus:ring-red-500"
                    />
                ...
                <div class="flex gap-3 border-t border-slate-100 bg-slate-50 p-4">
                    <button
                        type="button"
                        onclick="closeRejectModal()"
                        class="flex-1 rounded-xl border border-slate-200 bg-white py-3 text-[10px] font-black tracking-widest text-slate-500 uppercase transition-all hover:bg-slate-100"
                    >
                        Kanselahin
                    </button>
                    <button
                        type="submit"
                        class="flex-1 rounded-xl bg-red-600 py-3 text-[10px] font-black tracking-widest text-white uppercase shadow-md transition-all hover:bg-red-700 active:scale-95"
                    >
                        I-Submit Reject
                    </button>
                </div>
    ...
    >
        ...
        >
            ...
            >
                <a
                    href="{{ route('admin.queue.print_logbook') }}?download=1"
                    class="flex items-center gap-2 rounded-xl bg-red-600 px-6 py-3 text-[10px] font-black tracking-widest text-white uppercase shadow-sm transition-all hover:bg-red-700 active:scale-95"
                >
                    <svg class="h-4 w-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4 16v1a3 3 0 003 3h10a3 3 0 003-3v-1m-4-4l-4 4m0 0l-4-4m4 4V4"></path></svg>
