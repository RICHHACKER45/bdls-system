...
@section ('content')
    ...
    <div id="tab-pending" class="tab-content {{ session('active_tab', 'pending') == 'pending' ? 'block' : 'hidden' }}">
        ...
        <div class="mb-6 rounded-xl border border-slate-200 bg-white p-4 shadow-sm">
            <form method="GET" action="{{ route('admin.dashboard') }}" class="flex w-full min-w-0 flex-wrap gap-2">
                <input type="text" name="search" value="{{ request('search') }}" placeholder="Maghanap ng pangalan o numero..." class="min-w-[200px] flex-1 rounded-lg border border-slate-300 px-4 py-2 outline-none focus:ring-2 focus:ring-slate-900 focus:outline-none" />
                ...
                <button type="submit" class="shrink-0 rounded-lg bg-slate-900 px-6 py-2 font-bold text-white transition-all hover:bg-slate-800 active:scale-95">Maghanap</button>
            </form>
        </div>
        ...
        <div id="sub-pending" class="sub-tab-content grid grid-cols-1 gap-6 xl:grid-cols-2">
                    ...
                    <div class="flex flex-1 flex-col justify-between">
                        <div>
                            ...
                            <p class="mb-3 flex flex-wrap gap-x-2 gap-y-1 text-[11px] font-bold tracking-widest text-slate-500 uppercase">
                                ...
                                <span>{{ $user->age }} TAONG GULANG</span>
                                <span class="text-slate-300">|</span>
            ...
            @empty
                <p class="col-span-full py-10 text-center font-bold text-slate-500 italic">Walang mga nakabinbing pagpaparehistro.</p>
            @endforelse
        ...
        <div id="sub-approved" class="sub-tab-content hidden space-y-3">
            @forelse ($approvedAccounts as $user)
                <details class="group overflow-hidden rounded-xl border border-slate-200 bg-white shadow-sm">
                    ...
                    <div class="grid grid-cols-1 gap-6 border-t border-slate-50 bg-slate-50/30 p-5 text-xs md:grid-cols-2 lg:grid-cols-3">
                        <div>
                            <p class="mb-1 font-black tracking-widest text-slate-400 uppercase">Mga Detalye ng Contact</p>
                            <p class="font-bold text-slate-900">{{ $user->contact_number }}</p>
                            <p class="font-medium text-slate-500">{{ $user->email ?? 'Walang binigay na email' }}</p>
                        </div>
                        <div>
                            <p class="mb-1 font-black tracking-widest text-slate-400 uppercase">Personal na Impormasyon</p>
                            <p class="font-bold text-slate-900 uppercase">{{ $user->sex }} | {{ $user->age }} TAONG GULANG</p>
                            <p class="font-medium text-slate-500 italic">DOB: {{ $user->date_of_birth->format('M d, Y') }}</p>
                        </div>
                        <div>
                            <p class="mb-1 font-black tracking-widest text-slate-400 uppercase">Tirahan</p>
                        ...
                        <div class="flex items-center justify-between border-t border-slate-100 pt-2 lg:col-span-3">
                            <p class="text-[9px] font-bold tracking-widest text-slate-400 uppercase">Tinanggap ang mga Kasunduan: {{ $user->terms_accepted_at ? $user->terms_accepted_at->format('M d, Y h:i A') : 'N/A' }}</p>
                            <div class="flex items-center gap-3">
                                <div class="flex gap-2">
                                ...
                                <div class="flex items-center gap-2 border-l border-slate-200 pl-3">
                                    <button type="button" onclick="openSuspendModal('{{ $user->id }}', '{{ $user->first_name }} {{ $user->last_name }}')" class="rounded border border-amber-200 bg-amber-50 px-3 py-1.5 text-[9px] font-black tracking-widest text-amber-600 uppercase shadow-sm transition-all hover:bg-amber-100 active:scale-95">I-suspend</button>
            ...
            @empty
                <p class="py-10 text-center font-bold text-slate-500 italic">Walang mga aprubadong account.</p>
            @endforelse
                    ...
                    <div>
                        <div class="mb-4 flex items-start justify-between">
                            <div>
                                ...
                                <p class="text-[10px] font-bold tracking-widest text-slate-500 uppercase">{{ $user->contact_number }} | Mga Pagsubok: {{ $user->rejection_count }}/5</p>
                            </div>
                            <span class="rounded bg-red-100 px-2 py-1 text-[9px] font-black tracking-widest text-red-700 uppercase shadow-sm">Tinanggihan</span>
                        </div>

                        <div class="mb-4 rounded-lg border border-red-100 bg-red-50 p-3">
                            <p class="mb-1 text-[9px] font-black tracking-widest text-red-400 uppercase italic">Rason ng Pagtanggi</p>
                            <p class="text-xs leading-snug font-bold text-red-700">{{ $user->rejection_reason }}</p>
                        </div>
                    </div>

                    <button type="button" onclick="openDeleteModal('{{ $user->id }}')" class="w-full rounded-lg bg-red-600 py-2.5 text-[10px] font-black tracking-widest text-white uppercase shadow-md transition-all hover:bg-red-700 active:scale-95">Burahin ang Account</button>
                </div>
            @empty
                <p class="col-span-full py-10 text-center font-bold text-slate-500 italic">Walang mga tinanggihang account.</p>
            @endforelse
    ...
    <div id="tab-queue" class="tab-content {{ session('active_tab') == 'queue' ? 'block' : 'hidden' }}">
        <div class="mt-6 mb-6 flex gap-2 overflow-x-auto pb-2">
            <button onclick="showSubTab('queue-active', this)" class="sub-tab-btn rounded-full border border-slate-900 bg-slate-900 px-5 py-2 text-sm font-bold whitespace-nowrap text-white transition-all">Aktibong Pila ({{ $activeQueue->count() }})</button>
            <button onclick="showSubTab('queue-received', this)" class="sub-tab-btn rounded-full border border-transparent bg-slate-200 px-5 py-2 text-sm font-bold whitespace-nowrap text-slate-700 transition-all hover:bg-slate-300">Kasaysayan ng Tinanggap ({{ $receivedQueue->count() }})</button>
        </div>
                ...
                <tbody class="divide-y divide-slate-100">
                    @forelse ($activeQueue as $queue)
                        @php
                            ...
                            if($rawStatus == 'pending') { $nextStatus = 'processing'; $btnLabel = 'Iproseso ang Request'; }
                            elseif($rawStatus == 'processing') {
                                if(in_array($queue->document_type_id, $interviewDocs)) { 
                                    $nextStatus = 'for_interview'; $btnLabel = 'I-set para sa Interview'; 
                                } else { 
                                    $nextStatus = 'released'; $btnLabel = 'I-release ang Dokumento'; 
                                }
                            }
                            elseif($rawStatus == 'for_interview') { $nextStatus = 'released'; $btnLabel = 'I-release ang Dokumento'; }
                            elseif($rawStatus == 'released') { $nextStatus = 'received'; $btnLabel = 'I-marka bilang Tinanggap'; }
                        @endphp
                        <tr class="transition-colors hover:bg-slate-50">
                            <td class="p-4 text-xl font-black tracking-tighter text-slate-900">{{ $queue->queue_number }}</td>
                            <td class="p-4">
                                <p class="mb-1 text-sm leading-none font-bold text-slate-900 uppercase">{{ $user->last_name }}, {{ $user->first_name }}</p>
                                <p class="font-mono text-[10px] tracking-tight text-slate-500">{{ $user->contact_number }}</p>
                            </td>
                            ...
                            <td class="flex justify-end gap-2 p-4 text-right">
                                ...
                                @if ($rawStatus === 'pending' || $rawStatus === 'processing')
                                    <button type="button" onclick="openStatusModal('{{ $queue->id }}', 'rejected', 'Tanggihan ang Request')" class="rounded-lg border border-red-200 bg-red-50 px-4 py-2 text-[10px] font-black tracking-widest text-red-600 uppercase shadow-sm transition-all hover:bg-red-100 active:scale-95">Tanggihan</button>
                                @endif

                                @if ($btnLabel === '' && $rawStatus !== 'pending' && $rawStatus !== 'processing')
                                    <span class="mt-2 text-xs font-bold text-slate-400 italic">Walang Aksyon</span>
                                @endif
                            </td>
                        </tr>
                    @empty
                        <tr>
                            <td colspan="5" class="p-12 text-center font-bold text-slate-400 italic">Walang aktibong nakapila sa ngayon.</td>
                        </tr>
                    @endforelse
                </tbody>
            </table>
        ...
        <div id="queue-received" class="sub-tab-content hidden overflow-hidden rounded-xl border border-slate-200 bg-white shadow-sm">
            <div class="flex items-center justify-between border-b border-slate-100 bg-slate-50 p-4">
                <h3 class="text-sm font-bold tracking-widest text-slate-800 uppercase">Mga Record na Na-release</h3>
            ...
            <table class="w-full border-collapse text-left">
                <thead>
                    <tr class="border-b border-slate-200 bg-slate-50 text-[10px] tracking-[0.15em] text-slate-500 uppercase">
                        ...
                        <th class="p-4 font-black">Residente</th>
                        <th class="p-4 font-black">Dokumento</th>
                        <th class="p-4 font-black">Petsa ng Pag-release</th>
                    ...
                    @forelse ($receivedQueue as $queue)
                        <tr class="bg-slate-50/50">
                            ...
                            <td class="p-4 opacity-75">
                                <p class="mb-1 text-sm leading-none font-bold text-slate-900 uppercase">{{ $user->last_name }}, {{ $user->first_name }}</p>
                                <p class="font-mono text-[10px] text-slate-500">{{ $user->contact_number }}</p>
                            </td>
                            ...
                            <td class="p-4 text-right">
                                <span class="rounded-full border border-green-200 bg-green-100 px-3 py-1 text-[9px] font-black tracking-widest text-green-700 uppercase"> Tinanggap </span>
                            </td>
                        </tr>
                    @empty
                        <tr>
                            <td colspan="5" class="p-12 text-center font-bold text-slate-400 italic">Walang record ng kasaysayan ng pag-release.</td>
                        </tr>
                    @endforelse
                </tbody>
            </table>
    ...
    <div id="tab-walkin" class="tab-content {{ session('active_tab') == 'walkin' ? 'block' : 'hidden' }}">
        <div class="mx-auto max-w-3xl rounded-xl border border-slate-200 bg-white p-6 shadow-sm md:p-8">
            <div class="mb-6 text-center">
                ...
                <h2 class="text-2xl font-black tracking-tight text-slate-900 uppercase">Paghahanap ng Walk-in</h2>
                ...
                <div class="mt-8 border-t border-slate-100 pt-6">
                    @if (session('walkin_user'))
                        <form action="{{ route('admin.walkin.store') }}" method="POST" class="rounded-xl border border-green-200 bg-green-50 p-5">
                            ...
                            <div class="mb-4">
                                <p class="mb-1 text-[10px] font-black tracking-widest text-green-600 uppercase">Record na Nahanap</p>
                                ...
                                <p class="text-xs font-bold text-slate-500">{{ session('walkin_user')->sex }} | {{ session('walkin_user')->age }} TAONG GULANG</p>
                            </div>
                            ...
                            <div class="flex flex-col gap-3 sm:flex-row">
                                ...
                                <button type="submit" class="shrink-0 rounded-lg bg-green-600 px-6 py-3 text-[10px] font-black tracking-widest whitespace-nowrap text-white uppercase shadow-sm transition-all hover:bg-green-700 active:scale-95">Gumawa ng Request</button>
                            </div>
                        </form>
                    @else
                        <form action="{{ route('admin.walkin.store') }}" method="POST" class="rounded-xl border border-amber-200 bg-amber-50 p-5 md:p-6">
                            ...
                            <div class="mb-4 grid grid-cols-1 gap-3 sm:grid-cols-2">
                                ...
                                <input type="text" name="house_number" placeholder="Numero ng Bahay *" required class="w-full rounded-lg border border-amber-300 bg-white px-4 py-2.5 text-sm font-bold outline-none focus:ring-2 focus:ring-amber-600" />
                                <input type="text" name="purok_street" placeholder="Purok/Kalye *" required class="w-full rounded-lg border border-amber-300 bg-white px-4 py-2.5 text-sm font-bold outline-none focus:ring-2 focus:ring-amber-600" />
                            </div>
                            ...
                            <div class="mt-2 flex flex-col gap-3 border-t border-amber-200 pt-4 sm:flex-row">
                                ...
                                <button type="submit" class="shrink-0 rounded-lg bg-amber-600 px-6 py-3 text-[10px] font-black tracking-widest whitespace-nowrap text-white uppercase shadow-sm transition-all hover:bg-amber-700 active:scale-95">I-rehistro at Gumawa</button>
                            </div>
                        </form>
                    @endif
                </div>
        ...
        <div class="mx-auto mt-8 max-w-3xl">
            <div class="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm md:p-8">
                <div class="mb-2 flex items-center gap-3">
                    ...
                    <h2 class="text-2xl font-black tracking-tight text-slate-900 uppercase">I-broadcast ang Anunsyo</h2>
                </div>
            ...
            @endphp
                ...
                @if ($isCurfew || $errors->has('curfew'))
                    <div class="mb-6 rounded-r-xl border-l-4 border-red-500 bg-red-50 p-4 shadow-sm">
                        <div class="mb-1 flex items-center gap-2 text-xs font-black tracking-widest text-red-700 uppercase">
                            ...
                            Aktibo ang NTC SMS Curfew
                        </div>
                        <p class="text-sm font-bold text-red-600">Pansamantalang naka-disable ang Broadcast (9:00 PM - 7:00 AM) upang sumunod sa mga panuntunan laban sa Spam. Subukan muli bukas.</p>
                    </div>
                @endif
                ...
                <form action="{{ route('admin.announcements.broadcast') }}" method="POST">
                    @csrf
                    <div class="mb-4">
                        <label class="mb-2 block text-[10px] font-black tracking-widest text-slate-400 uppercase">Katawan ng Mensahe (Message Body) <span class="text-red-500">*</span></label>
    ...
    <script>
        function updateCharCount(textarea) {
            ...
            counter.innerText = `Mga Karakter: ${length} (Tinatayang ${credits} Credit/s bawat user)`;
    ...
    <div id="tab-audit" class="tab-content {{ session('active_tab') == 'audit' ? 'block' : 'hidden' }}">
        <div class="mt-6 mb-6 flex gap-2 overflow-x-auto pb-2">
            <button onclick="showSubTab('sub-audit-trail', this)" class="sub-tab-btn rounded-full border border-slate-900 bg-slate-900 px-5 py-2 text-sm font-bold whitespace-nowrap text-white transition-all">Audit Trail ng System</button>
            <button onclick="showSubTab('sub-notif-history', this)" class="sub-tab-btn rounded-full border border-transparent bg-slate-200 px-5 py-2 text-sm font-bold whitespace-nowrap text-slate-700 transition-all hover:bg-slate-300">Kasaysayan ng Notipikasyon</button>
            <button onclick="showSubTab('sub-generate-pdf', this)" class="sub-tab-btn rounded-full border border-transparent bg-slate-200 px-5 py-2 text-sm font-bold whitespace-nowrap text-slate-700 transition-all hover:bg-slate-300">Gumawa ng Analytics</button>
        </div>
                ...
                <table class="relative w-full border-collapse text-left">
                    <thead class="sticky top-0 z-10">
                        <tr class="border-b border-slate-200 bg-slate-100 text-[10px] tracking-[0.15em] text-slate-500 uppercase">
                            <th class="p-4 font-black">Petsa at Oras</th>
                    ...
                    <tbody class="divide-y divide-slate-100">
                        ...
                        @empty
                            <tr>
                                <td colspan="4" class="p-12 text-center font-bold text-slate-400 italic">Wala pang naitalang aksyon sa system.</td>
                            </tr>
                        @endforelse
                    </tbody>
                </table>
        ...
        <div id="sub-notif-history" class="sub-tab-content hidden overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm">
            <div class="border-b border-slate-200 bg-slate-50 p-6">
                <h2 class="text-xl font-black tracking-tight text-slate-900 uppercase">Kasaysayan ng Notipikasyon</h2>
            </div>
            <div class="max-h-[600px] overflow-x-auto overflow-y-auto">
                <table class="relative w-full border-collapse text-left">
                    <thead class="sticky top-0 z-10">
                    ...
                    <tbody class="divide-y divide-slate-100">
                        ...
                        @empty
                            <tr>
                                <td colspan="5" class="p-12 text-center font-bold text-slate-400 italic">Walang record ng mga notipikasyon.</td>
                            </tr>
                        @endforelse
                    </tbody>
                </table>
        ...
        <div id="sub-generate-pdf" class="sub-tab-content hidden overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm">
            <div class="border-b border-slate-200 bg-slate-50 p-6">
                <h2 class="text-xl font-black tracking-tight text-slate-900 uppercase">Gumawa ng System Analytics</h2>
            </div>
                    ...
                    <div class="grid grid-cols-1 gap-4 sm:grid-cols-2">
                        <div class="rounded-xl border border-slate-200 bg-slate-50 p-4">
                            ...
                            <select name="report_month" required class="w-full cursor-pointer rounded-lg border border-slate-300 bg-white px-4 py-2.5 font-bold text-slate-700 outline-none focus:ring-2 focus:ring-slate-900">
                                <option value="all">Buong Taon (Lahat ng Buwan)</option>
                                <option value="01">Enero</option>
                                <option value="02">Pebrero</option>
                                <option value="03" selected>Marso</option>
    ...
    <div id="statusModal" class="fixed inset-0 z-[100] flex hidden items-center justify-center bg-slate-900/80 p-4 backdrop-blur-sm">
        <div class="w-full max-w-sm transform overflow-hidden rounded-2xl border border-slate-100 bg-white p-6 shadow-2xl transition-all">
            <div class="mb-6 flex flex-col items-center text-center">
                ...
                <h3 id="statusModalTitle" class="text-xl font-black tracking-tight text-slate-900 uppercase">Ilipat sa <span id="nextStatusLabel" class="text-blue-600">...</span>?</h3>
            ...
            <form id="statusForm" method="POST">
                ...
                <div class="flex flex-col gap-3">
                    <button type="button" onclick="closeStatusModal()" class="w-full rounded-xl border border-slate-200 bg-white py-3 text-[10px] font-black tracking-widest text-slate-500 uppercase shadow-sm transition-all hover:bg-slate-50 hover:text-slate-900 active:scale-95">Kanselahin</button>
                    <button type="submit" id="statusSubmitBtn" class="w-full rounded-xl bg-slate-900 py-3 text-xs font-black tracking-widest text-white uppercase shadow-md transition-all hover:bg-slate-800 active:scale-95">I-kumpirma ang Pagbabago</button>
                </div>
            </form>
        </div>
    </div>
    ...
    <div id="deleteModal" class="fixed inset-0 z-[110] flex hidden items-center justify-center bg-slate-900/80 p-4 backdrop-blur-sm">
        <div class="w-full max-w-sm transform overflow-hidden rounded-2xl border border-red-100 bg-white p-6 shadow-2xl transition-all">
            <div class="mb-6 flex flex-col items-center text-center">
                ...
                <h3 class="text-xl font-black tracking-tight text-slate-900 uppercase">Burahin ang Account?</h3>
                <p class="mt-2 px-4 text-sm font-medium text-slate-500">Babala: Ang lahat ng data pati litrato ay permanenteng mawawala. Hindi na ito maibabalik.</p>
            </div>
            ...
            <form id="deleteForm" method="POST">
                ...
                @method ('DELETE')
                <div class="flex flex-col gap-3">
                    <button type="submit" class="w-full rounded-xl bg-red-600 py-3 text-xs font-black tracking-widest text-white uppercase shadow-md transition-all hover:bg-red-700 active:scale-95">Burahin nang Permanente</button>
                    <button type="button" onclick="closeDeleteModal()" class="w-full py-2 text-[10px] font-black tracking-widest text-slate-400 uppercase transition-all hover:text-slate-600">Kanselahin</button>
                </div>
            </form>
        </div>
    </div>
    ...
    <div id="rejectModal" class="fixed inset-0 z-[110] hidden items-center justify-center bg-slate-900/80 p-4 backdrop-blur-sm">
        <div class="w-full max-w-md transform overflow-hidden rounded-2xl border border-red-100 bg-white shadow-2xl transition-all">
            ...
            <form id="rejectForm" method="POST" action="">
                @csrf
                <div class="p-6">
                    <p class="mb-4 text-sm font-medium text-slate-600">Ita-type mo ang rason kung bakit tinanggihan si <strong id="rejectUserName" class="text-slate-900"></strong>. Ipapadala ito via SMS.</p>

                    <label class="mb-2 block text-[10px] font-black tracking-[0.2em] text-slate-400 uppercase">Rason ng Pagtanggi</label>
                    <input type="text" name="rejection_reason" maxlength="60" required placeholder="Hal: Malabo ang ID, paki-upload nang maayos." class="w-full rounded-xl border-2 border-slate-100 p-3 font-bold text-slate-900 placeholder-slate-300 transition-all outline-none focus:border-red-500 focus:ring-2 focus:ring-red-500" />
                ...
                <div class="flex gap-3 border-t border-slate-100 bg-slate-50 p-4">
                    ...
                    <button type="submit" class="flex-1 rounded-xl bg-red-600 py-3 text-[10px] font-black tracking-widest text-white uppercase shadow-md transition-all hover:bg-red-700 active:scale-95">I-Submit ang Pagtanggi</button>
                </div>
            </form>
        </div>
    </div>
    ...
    <div id="suspendModal" class="fixed inset-0 z-[200] flex hidden items-center justify-center bg-slate-900/80 p-4 backdrop-blur-sm transition-opacity">
        <div class="w-full max-w-sm transform overflow-hidden rounded-2xl border border-amber-100 bg-white p-6 shadow-2xl transition-all">
            <div class="mb-6 flex flex-col items-center text-center">
                ...
                <h3 class="text-xl font-black tracking-tight text-slate-900 uppercase">I-suspend ang Account?</h3>
            ...
            <form id="suspendForm" method="POST">
                ...
                <div class="flex flex-col gap-3">
                    ...
                    <button type="button" onclick="closeSuspendModal()" class="w-full py-2 text-[10px] font-black tracking-widest text-slate-400 uppercase transition-all hover:text-slate-600">Kanselahin</button>
                </div>
            </form>
        </div>
    </div>
            ...
            <div class="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm md:p-8">
                ...
                <form action="{{ route('password.update') }}" method="POST" class="space-y-5">
                    ...
                    <div class="grid grid-cols-1 gap-5 sm:grid-cols-2">
                        ...
                        <div>
                            <label class="mb-2 block text-[10px] font-black tracking-widest text-slate-400 uppercase">I-kumpirma ang Password <span class="text-red-500">*</span></label>
