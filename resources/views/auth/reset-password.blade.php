        ...
        <form action="{{ route('password.update.submit') }}" method="POST" class="space-y-6">
            ...
            <div class="grid grid-cols-1 gap-4 sm:grid-cols-2">
                <div>
                    ...
                    <input type="password" name="password" minlength="8" required class="w-full rounded-lg border border-slate-300 bg-slate-50 px-4 py-3 transition-all outline-none focus:border-slate-900 focus:ring-2 focus:ring-slate-900" />
                </div>
                <div>
                    <label class="mb-1 block text-sm font-semibold text-slate-700">Confirm Password</label>
                    <input type="password" name="password_confirmation" minlength="8" required class="w-full rounded-lg border border-slate-300 bg-slate-50 px-4 py-3 transition-all outline-none focus:border-slate-900 focus:ring-2 focus:ring-slate-900" />
                </div>
            </div>

            <button type="submit" class="mt-4 w-full rounded-xl bg-red-600 px-8 py-3.5 font-bold text-white shadow-md transition-all hover:bg-red-700 active:scale-95">I-save at Mag-login</button>
        </form>

        <div class="mt-6 text-center">
            <a href="{{ route('login') }}" class="text-xs font-bold text-slate-400 transition-all hover:text-slate-600">I-cancel at bumalik sa Login</a>
        </div>
    </div>
