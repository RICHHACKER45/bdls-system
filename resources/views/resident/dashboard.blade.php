...
@section ('content')
        ...
        @if (Auth::user()->email && !Auth::user()->email_verified_at)
            <div class="mb-8 flex flex-col items-start justify-between gap-4 rounded-r-xl border-l-4 border-blue-500 bg-blue-50 p-4 shadow-sm sm:flex-row sm:items-center sm:p-5">
                <div>
                    ...
                    <p class="mt-1 text-sm text-blue-600">Kasalukuyang naka-link ang <span class="font-bold">{{ Auth::user()->email }}</span>. I-verify ito para makatanggap ng mga digital receipt.</p>
                </div>
            ...
            @if (Auth::user()->rejection_count > 0)
                ...
                <div class="mb-6 rounded-r-xl border-l-4 border-red-500 bg-red-50 p-6 shadow-sm">
                    <div class="mb-2 flex items-center gap-3 text-lg font-bold text-red-800">
                        ...
                        Tinanggihan ang Pagpaparehistro
                    </div>
                    <p class="mb-4 text-sm text-red-700">Ang iyong request ay tinanggihan. Dahilan: <span class="font-bold underline">{{ Auth::user()->rejection_reason }}</span>. Mayroon ka pang {{ 5 - Auth::user()->rejection_count }} pagkakataon para mag-request.</p>
                    <button type="button" onclick="openResubmitModal()" class="rounded-lg bg-red-600 px-5 py-2.5 text-xs font-bold tracking-widest text-white uppercase shadow-sm transition-all hover:bg-red-700 active:scale-95">I-upload muli ang mga Kinakailangan</button>
                </div>
            @else
                <!-- Under Review Alert -->
                <div class="mb-6 rounded-r-xl border-l-4 border-amber-500 bg-amber-50 p-6 shadow-sm">
                    <div class="mb-2 flex items-center gap-3 text-lg font-bold text-amber-800">
                        ...
                        Ang Account ay Sinusuri Pa
                    </div>
                    <p class="text-sm text-amber-700">Kasalukuyang sinusuri ng Admin ng Barangay ang iyong Valid ID at Selfie. Hindi ka pa maaaring mag-request ng dokumento hangga't hindi ito naaprubahan.</p>
                </div>
            @endif
        @else
            <div class="grid grid-cols-1 gap-4 lg:grid-cols-3">
                <!-- LEFT COLUMN: Request Card + Announcements -->
                <div class="col-span-1 flex flex-col gap-4 lg:col-span-2">
                    ...
                    <div class="flex flex-col justify-between rounded-2xl border border-slate-100 bg-white p-6 shadow-sm md:p-8">
                        <div>
                            ...
                            <p class="mb-8 max-w-md text-slate-500">Mag-request ng Barangay Clearance, Certificate of Indigency, at iba pang mga dokumento nang hindi na pumipila nang matagal.</p>
                        </div>
                        <div>
                            @if (Auth::user()->locked_until && Auth::user()->locked_until > now())
                                <div class="rounded-xl border border-amber-200 bg-amber-50 p-4 text-center">
                                    ...
                                    <p class="mt-1 text-xs text-amber-600">Maaari kang muling mag-request sa: <br />
                ...
                <div class="flex flex-col gap-4">
                    <!-- 1. ACTIVE REQUESTS SUMMARY -->
                    <div class="rounded-2xl border border-slate-100 bg-white p-6 shadow-sm">
                        <h3 class="mb-4 font-bold text-slate-700">Mga Aktibong Request</h3>
                        <div class="grid grid-cols-3 gap-2 text-center">
                            ...
                            >
                                ...
                                <span class="mt-1 text-[8px] font-bold tracking-widest text-blue-700 uppercase">Proseso</span>
                            </button>
                            <button
                                onclick="
                                    switchTab('tracking');
                            ...
                            >
                                ...
                                <span class="mt-1 text-[8px] font-bold tracking-widest text-orange-700 uppercase">Handa Na</span>
                            </button>
                        </div>
                        <p class="mt-4 text-center text-[10px] font-medium text-slate-400">I-click ang mga numero upang makita ang detalye.</p>
                    </div>

                    <!-- 2. RECENT HISTORY BOX -->
                    <div class="rounded-2xl border border-slate-100 bg-white p-6 shadow-sm">
                        <h3 class="mb-4 font-bold text-slate-700">Kamakailang Kasaysayan</h3>
                        <div class="space-y-3">
    ...
    <div id="tab-settings" class="tab-content {{ session('active_tab') == 'settings' ? 'block' : 'hidden' }}">
        <h1 class="mb-6 text-2xl font-bold text-slate-900">Mga Setting ng Account</h1>
        ...
        <div class="space-y-8 rounded-2xl border border-slate-100 bg-white p-6 shadow-sm md:p-8">
            ...
            <!-- CONTACT NUMBER BOX -->
            <div>
                <h2 class="mb-3 text-lg font-bold text-slate-900">Pangunahing Contact Number</h2>
                    ...
                    @if (Auth::user()->contact_verified_at)
                        ...
                        @else
                            <span class="flex items-center justify-center gap-1 text-[10px] font-bold text-slate-400 italic sm:justify-start">
                                ...
                                Naka-lock hanggang {{ Auth::user()->contact_verified_at->copy()->addDays(30)->format('M d') }}
                            </span>
                        @endif
                    @else
                        ...
                        <button onclick="openSettingsModal('verifyContactModal')" class="w-full animate-pulse rounded-lg bg-red-600 px-6 py-2.5 text-sm font-bold text-white transition-all hover:bg-red-700 active:scale-95 sm:w-auto">I-verify ang OTP</button>
                        <button onclick="openSettingsModal('changeContactModal')" class="w-full rounded-lg border border-slate-300 bg-white px-6 py-2.5 text-sm font-bold text-slate-700 transition-all hover:bg-slate-50 active:scale-95 sm:w-auto">Palitan</button>
                    @endif
                </div>
                <p class="mt-2 text-xs text-slate-500">Ito ang iyong login ID at channel para sa mga SMS update.</p>
            </div>
            ...
            <div>
                    ...
                    @if (Auth::user()->email)
                        @if (Auth::user()->email_verified_at)
                            ...
                            @else
                                <span class="flex items-center justify-center gap-1 text-[10px] font-bold text-slate-400 italic sm:justify-start">
                                    <svg class="h-3 w-3" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 15v2m-6 4h12a2 2 0 002-2v-6a2 2 0 00-2-2H6a2 2 0 00-2 2v6a2 2 0 002 2zm10-10V7a4 4 0 00-8 0v4h8z"></path></svg>
                                    Naka-lock hanggang {{ Auth::user()->email_verified_at->copy()->addDays(30)->format('M d') }}
                                </span>
                            @endif
                        @else
                            <span class="inline-flex w-full items-center justify-center gap-1 rounded-full bg-amber-100 px-3 py-1.5 text-xs font-bold text-amber-700 sm:w-auto">Unverified</span>
                            <button onclick="openSettingsModal('verifyEmailModal')" class="w-full animate-pulse rounded-lg bg-red-600 px-6 py-2.5 text-sm font-bold text-white transition-all hover:bg-red-700 active:scale-95 sm:w-auto">I-verify ang OTP</button>
                        @endif
                    @else
                ...
                <p class="mt-2 text-xs text-slate-500">Opsyonal: Para makatanggap ng kopya ng mga digital receipt.</p>
            </div>
            ...
            <!-- PASSWORD SECURITY -->
            <div>
                <h2 class="mb-4 text-lg font-bold text-slate-900">Mga Setting ng Seguridad</h2>
                <form action="{{ route('password.update') }}" method="POST" class="max-w-md space-y-4">
                    ...
                    <div class="grid grid-cols-1 gap-4 sm:grid-cols-2">
                        ...
                        <div>
                            <label class="mb-1 block text-sm font-bold text-slate-700">I-kumpirma ang Password</label>
        ...
        <div class="w-full max-w-sm rounded-2xl bg-white p-6 shadow-2xl">
            ...
            <form action="{{ route('resident.settings.update_contact') }}" method="POST">
                ...
                <div class="flex gap-2">
                    <button type="button" onclick="closeSettingsModal('changeContactModal')" class="flex-1 rounded-xl bg-slate-200 py-3 text-xs font-black tracking-widest text-slate-700 uppercase transition-all hover:bg-slate-300 active:scale-95">Kanselahin</button>
    ...
    <div id="verifyContactModal" class="fixed inset-0 z-50 flex hidden items-center justify-center bg-slate-900/80 p-4 backdrop-blur-sm">
        <div class="w-full max-w-sm rounded-2xl bg-white p-6 shadow-2xl">
            <h3 class="mb-2 text-lg font-black text-slate-900 uppercase">I-verify ang Numero</h3>
            ...
            <form action="{{ route('resident.settings.verify_contact') }}" method="POST">
                ...
                <div class="mb-4 flex gap-2">
                    <button type="button" onclick="closeSettingsModal('verifyContactModal')" class="flex-1 rounded-xl bg-slate-200 py-3 text-xs font-black tracking-widest text-slate-700 uppercase transition-all hover:bg-slate-300 active:scale-95">Kanselahin</button>
                    <button type="submit" class="flex-1 rounded-xl bg-red-600 py-3 text-xs font-black tracking-widest text-white uppercase shadow-md transition-all hover:bg-red-700 active:scale-95">I-verify ang OTP</button>
                </div>
            </form>
            ...
            <div class="border-t border-slate-100 pt-4 text-center">
                <form action="{{ route('resident.settings.update_contact') }}" method="POST" id="resendContactOtpForm">
                    ...
                    <button type="submit" id="resendContactBtn" class="text-xs font-bold text-slate-500 transition-all hover:text-slate-900 hover:underline">I-resend ang SMS Code <span id="timerContactDisplay" class="font-mono text-red-600"></span></button>
                </form>
            </div>
        </div>
    </div>
    ...
    <div id="changeEmailModal" class="fixed inset-0 z-[100] flex hidden items-center justify-center bg-slate-900/80 p-4 backdrop-blur-sm">
        <div class="w-full max-w-sm rounded-2xl bg-white p-6 shadow-2xl">
            ...
            <form action="{{ route('resident.email.add') }}" method="POST">
                ...
                <div class="flex gap-2">
                    <button type="button" onclick="closeSettingsModal('changeEmailModal')" class="flex-1 rounded-xl bg-slate-200 py-3 text-xs font-black tracking-widest text-slate-700 uppercase transition-all hover:bg-slate-300 active:scale-95">Kanselahin</button>
                    <button type="submit" class="flex-1 rounded-xl bg-slate-900 py-3 text-xs font-black tracking-widest text-white uppercase shadow-md transition-all hover:bg-slate-800 active:scale-95">I-save</button>
                </div>
            </form>
        </div>
    ...
    <div id="verifyEmailModal" class="fixed inset-0 z-50 flex hidden items-center justify-center bg-slate-900/80 p-4 backdrop-blur-sm">
        <div class="w-full max-w-sm rounded-2xl bg-white p-6 shadow-2xl">
            <h3 class="mb-2 text-lg font-black text-slate-900 uppercase">I-verify ang Email</h3>
            ...
            <form action="{{ route('resident.email.verify') }}" method="POST">
                ...
                <div class="mb-4 flex gap-2">
                    <button type="button" onclick="closeSettingsModal('verifyEmailModal')" class="flex-1 rounded-xl bg-slate-200 py-3 text-xs font-black tracking-widest text-slate-700 uppercase transition-all hover:bg-slate-300 active:scale-95">Kanselahin</button>
                    <button type="submit" class="flex-1 rounded-xl bg-red-600 py-3 text-xs font-black tracking-widest text-white uppercase shadow-md transition-all hover:bg-red-700 active:scale-95">I-verify ang OTP</button>
                </div>
            </form>
            ...
            <div class="border-t border-slate-100 pt-4 text-center">
                <form action="{{ route('resident.email.send') }}" method="POST" id="resendOtpForm">
                    @csrf
                    <button type="submit" id="resendEmailBtn" class="text-xs font-bold text-slate-500 transition-all hover:text-slate-900 hover:underline">I-resend ang Email Code <span id="timerEmailDisplay" class="font-mono text-red-600"></span></button>
                </form>
            </div>
        </div>
    </div>
    ...
    <div id="tab-tracking" class="tab-content hidden">
        <h1 class="mb-6 text-2xl font-bold text-slate-900">I-track ang aking mga Request</h1>
        ...
        <div class="mb-6 flex gap-2 overflow-x-auto border-b border-slate-100 pb-2">
            ...
            <button id="btn-track-history" onclick="showResidentSubTab('track-history', this)" class="res-sub-tab-btn rounded-full bg-slate-100 px-5 py-2 text-sm font-bold whitespace-nowrap text-slate-600 transition-all hover:bg-slate-200">Tinanggap ({{ $myRequests->where('status', 'received')->count() }})</button>
            <button id="btn-track-rejected" onclick="showResidentSubTab('track-rejected', this)" class="res-sub-tab-btn rounded-full bg-slate-100 px-5 py-2 text-sm font-bold whitespace-nowrap text-slate-600 transition-all hover:bg-slate-200">Tinanggihan / Kinansela ({{ $myRequests->whereIn('status', ['rejected', 'canceled'])->count() }})</button>
        </div>
        ...
        <div id="track-pending" class="res-sub-tab-content block space-y-4">
            ...
            @empty
                <div class="rounded-xl border border-slate-200 bg-slate-50 p-12 text-center">
                    <p class="font-bold text-slate-500 italic">Wala kang nakabinbing na request.</p>
                </div>
            @endforelse
        </div>
        ...
        <div id="track-history" class="res-sub-tab-content hidden space-y-4">
            ...
            @empty
                <div class="rounded-xl border border-slate-200 bg-slate-50 p-12 text-center">
                    <p class="font-bold text-slate-500 italic">Wala ka pang nakaraang transaksyon.</p>
                </div>
            @endforelse
        </div>
    ...
    <div id="resubmitModal" class="fixed inset-0 z-[100] flex hidden items-center justify-center bg-slate-900/80 p-4 backdrop-blur-sm">
        <div class="w-full max-w-md transform overflow-hidden rounded-2xl border border-red-100 bg-white shadow-2xl transition-all">
            <div class="flex items-center justify-between border-b border-red-100 bg-red-50 p-4 text-red-700">
                <h3 class="text-lg font-black tracking-tight uppercase">I-resubmit ang mga Kinakailangan</h3>
            ...
            <form action="{{ route('resident.resubmit_registration') }}" method="POST" enctype="multipart/form-data" class="p-6">
                ...
                <div class="mb-6 space-y-4">
                    <div>
                        <label class="mb-2 block text-[10px] font-black tracking-widest text-slate-400 uppercase">I-upload ang Valid ID</label>
                    ...
                    <div>
                        <label class="mb-2 block text-[10px] font-black tracking-widest text-slate-400 uppercase">I-upload ang Selfie kasama ang ID</label>
                ...
                <div class="flex flex-col gap-2">
                    <button type="submit" class="w-full rounded-xl bg-slate-900 py-3.5 text-xs font-black tracking-widest text-white uppercase shadow-md transition-all hover:bg-slate-800 active:scale-95">I-submit para sa Re-review</button>
                    <button type="button" onclick="closeResubmitModal()" class="w-full py-2 text-[10px] font-black tracking-widest text-slate-400 uppercase">Kanselahin</button>
                </div>
            </form>
        </div>
    </div>
        ...
        <div class="flex max-h-[90vh] w-full max-w-2xl flex-col overflow-hidden rounded-2xl border border-slate-100 bg-white shadow-2xl">
            ...
            <div class="overflow-y-auto p-6">
                <form action="{{ route('resident.request.store') }}" method="POST" id="requestForm" class="space-y-6" enctype="multipart/form-data">
                    ...
                    <div>
                        <label class="mb-2 block text-sm font-bold text-slate-800">Karagdagang Detalye (Opsyonal)</label>
                    ...
                    <div class="flex justify-end gap-3 border-t border-slate-100 bg-slate-50 p-4">
                        ...
                        <button type="submit" class="rounded-xl bg-slate-900 px-8 py-2.5 font-bold text-white shadow-md hover:bg-slate-800 active:scale-95">I-submit ang Request</button>
                    </div>
                </form>
            </div>
        </div>
