...
* **Unified Digital Queue:** Pinagsamang pila para sa Walk-in at Online requests [5, 6].
* **Real-time Tracking:** Upang maiwasan ang pabalik-balik na pagtatanong ng mga residente na nakakaabala sa workflow ng barangay staff [7, 8].
* **Automated SMS Notification:** Ang pangunahing tulay ng komunikasyon na may optional Email fallback para sa digital receipts [6, 9].
...
* **No E-Payments:** Lahat ng serbisyo at dokumento ay ₱0.00 (Libre) [13-15]. Walang payment gateway integration [16].
...
* **NO LINKS:** Bawal ang clickable URLs, bit.ly, o website links [39].
* **Prefix Required:** Laging dapat mag-umpisa sa "Brgy Dona Lucia:" [39].
* **Night Curfew:** Bawal mag-send ng non-emergency/bulk SMS mula 9:00 PM hanggang 7:00 AM [39].
...
* `users`: Single Table Inheritance para sa admin, residente, at walk-in.
* `document_types`: Naglalaman ng 13 Barangay Requests at requirements_description nito.
* `service_requests`: Ang core transaction table (nakakabit sa user_id at document_type_id).
* `attachments`: Para sa extra uploaded files bukod sa Valid ID.
* `notification_logs`: Ang audit trail ng lahat ng SMS na ipinadala kasama ang delivery status.
* `audit_logs`: Taga-record ng mga actions ni Admin (Approve, Reject, Update Status).
* `announcements`: Para sa pag-broadcast ng mensahe sa lahat ng verified residents.
