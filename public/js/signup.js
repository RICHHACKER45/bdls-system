...
function validateAndGo(currentStepId, nextStepId) {
    ...
    inputs.forEach((input) => {
        ...
        } else if ((input.id === 'password' || input.id === 'password_confirmation') && input.value.length < 8) {
            hasError = true;
        ...
        } else if (input.id === 'password_confirmation' && input.value !== document.getElementById('password').value) {
            hasError = true;
...
document.addEventListener('DOMContentLoaded', function () {
    const formElements = document.querySelectorAll('input:not([type="password"]):not([type="file"]):not([type="checkbox"]), select');
